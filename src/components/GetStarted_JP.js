...
export default function GetStarted(){
    ...
    async function getTriviaData(){
        ...
        //  Translator(resp.data.results.map(result=>result.correct_answer))
        //     .then((res) => {
        ...
        // console.log(resp.data.results)
            // ! translate block
        TranslatorCorrectAns(resp.data.results, 'correct_answer', setCorrectAnswer)
            // ! translate block

        // setCorrectAnswer(Translator(resp.data.results.map(result=>result.correct_answer)).map(item=>item))
        const set = []
        for (let index = 0; index < resp.data.results.length; index++) {
            ...
            const day = await hi.map((result)=>result)


            // ! translate block
            const objectedAnswers =  day.map(item=>(
                { 
                value:item,
                id:nanoid(),
                isSelected:false
            ...
            // console.log(hey.translatedText)
            // const traQues = TranslatorPlane(element.question)
            // const damn = await traQues.then(result=>result)
            // console.log(damn)
            const question = {value:hey.translatedText, id:nanoid()}
            // ! translate block
